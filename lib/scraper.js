...
const EXTRACT_SCRIPT = `
  (() => {
    ...
    // Collect comment elements from both direct articles and inside dialogs (reels)
...
/**
 * Dismisses cookie consent / GDPR dialogs that block page content.
 */
async function dismissDialogs(page) {
  // Try Puppeteer native clicks first for known Facebook selectors
  const fbSelectors = [
  ...
  // Fallback: evaluate-based search
  await page.evaluate(() => {
    const patterns = [
...
export async function scrapePostComments(browser, postUrl, options = {}) {
  ...
  const run = async () => {
    ...
    try {
      ...
      // Step 4: Handle consent if needed
      ...
      finalUrl = page.url();
      console.log(`After nav - Title: "${title}", URL: ${finalUrl}`);

      const isReelNav = isReel || finalUrl.includes('/reel/');
      if (isReelNav) {
        console.log(`Reel URL detected. Final URL: ${finalUrl}`);
      ...
      // Load more comments by clicking "Diğer yorumları gör" / "View more comments"
