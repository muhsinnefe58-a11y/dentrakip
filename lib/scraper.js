...
async function scrapeAdPostComments(browser, postUrl, options = {}) {
  ...
  try {
    ...
    console.log(`Navigating to Ads Library: ${postUrl}`);
    await page.goto(postUrl, { waitUntil: 'networkidle0', timeout: 45000 }).catch(() => {});
    ...
    const adTitle = await page.title().catch(() => '');
    console.log(`Ads Library page title: "${adTitle}"`);

    let comments = [];

    // STRATEGY 1: Try to find the underlying post URL
    ...
    if (underlyingPostUrl) {
      ...
      await page.goto(underlyingPostUrl, { waitUntil: 'load', timeout: 30000 }).catch(() => {});
      await new Promise(r => setTimeout(r, 5000));

      const postTitle = await page.title().catch(() => '');
      const postFinalUrl = page.url();
      console.log(`Post page - Title: "${postTitle}", URL: ${postFinalUrl}`);
      ...
      if (postLoginWall) {
        console.log('Post page redirects to login. Trying direct extraction from ad page...');
      } else {
    ...
    // STRATEGY 2: If no post URL found or no comments from post, try direct extraction
    if (comments.length === 0) {
      console.log('Trying direct comment extraction from Ads Library page...');
      ...
      if (toggleClicked) {
        console.log('Comments toggle clicked on ad page');
    ...
    console.log(`Total comments extracted from ad: ${comments.length}`);
